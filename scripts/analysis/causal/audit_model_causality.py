...
def model_family(config: Mapping[str, Any]) -> str:
    ...
    if name == "MultiDAGCL":
        return "MultiDAGCL"
    from models.registry.causal import normalize_new_causal_model_name
...
def build_model(config: Dict[str, Any], family: str) -> torch.nn.Module:
    if family == "MMGCN":
        from scripts.models.mmgcn.unified.train import build_model as project_builder

        return project_builder(config)
    if family == "MultiDAGCL":
        from scripts.models.multidag_cl.unified.train import build_model as project_builder
...
def build_real_batch(
    ...
    if family == "MMGCN":
        from scripts.models.mmgcn.unified.train import build_dataloader

        loader = build_dataloader(config, split=split, shuffle=False)
    elif family == "MultiDAGCL":
        from scripts.models.multidag_cl.unified.train import build_dataloader
