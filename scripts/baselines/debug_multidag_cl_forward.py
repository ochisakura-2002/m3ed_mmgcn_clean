"""Fake-forward smoke test for the project MultiDAG+CL baseline port."""
...
def _has_future_leakage(adjacency: torch.Tensor) -> bool:
    seq_len = adjacency.size(1)
    future_mask = torch.triu(
        torch.ones(seq_len, seq_len, dtype=torch.bool, device=adjacency.device),
        diagonal=1,
    )
    return bool((adjacency.to(dtype=torch.bool) & future_mask.unsqueeze(0)).any().item())
...
def _run_case(
    ...
    future_leakage = _has_future_leakage(output["adjacency"])
    if future_leakage:
        raise AssertionError(f"{name} adjacency contains future leakage")
    ...
    print(f"[{name} fake] future leakage: {future_leakage}")
