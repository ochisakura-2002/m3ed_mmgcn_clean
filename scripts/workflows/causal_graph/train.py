...
from models.registry.causal import (  # noqa: E402
    build_new_causal_baseline,
    get_new_causal_model_family,
    normalize_new_causal_model_name,
)
from scripts.runtime.causal_graph import (  # noqa: E402
    build_dataloader,
    build_optimizer,
    compute_batch_loss,
    evaluate_model,
...
def run_training(
    ...
    if output_root_override is not None:
        config["output"]["output_base"] = str(Path(output_root_override))
        if config["output"].get("run_id"):
            config["output"].pop("root", None)
        else:
